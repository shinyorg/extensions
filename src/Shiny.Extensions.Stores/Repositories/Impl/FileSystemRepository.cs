...
namespace Shiny.Extensions.Stores.Repositories.Impl;
...
public class FileSystemRepository : IRepository
{
    ...
    public TEntity? Get<TEntity>(string identifier) where TEntity : IRepositoryEntity
    {
        ...
        {
            if (list.TryGetValue(identifier, out var entity))
                result = entity;
            else
    ...
    public void Clear<TEntity>() where TEntity : IRepositoryEntity
    {
        ...
        {
            list.Clear();
            foreach (var file in this.GetFiles<TEntity>())
                file.Delete();
    ...
    string GetPath<TEntity>(string identifier)
    {
        ...
        return Path.Combine(this.rootDir.FullName, fileName);
    }
    ...
    bool Write<TEntity>(TEntity entity) where TEntity : IRepositoryEntity
    {
        ...
        var value = this.serializer.Serialize<TEntity>(entity);
    ...
    void InTransaction<TEntity>(Action<Dictionary<string, TEntity>> action) where TEntity : IRepositoryEntity
    {
        ...
        lock (this.syncLock)
        {
            if (this.memory.TryGetValue(en, out var cached))
            {
                var entityDictionary = (Dictionary<string, TEntity>)cached;
