...
namespace Shiny.Extensions.Reflector.SourceGenerators;
...
public class ReflectorSourceGenerator : IIncrementalGenerator
{
    private static readonly DiagnosticDescriptor MissingPartialModifierRule = new DiagnosticDescriptor(
        ...
        category: "Shiny.Extensions.Reflector",
    ...
    static TypeDeclarationSyntax? GetSemanticTargetForGeneration(GeneratorSyntaxContext context)
    {
        ...
        foreach (var attributeListSyntax in typeDeclarationSyntax.AttributeLists)
        {
            foreach (var attributeSyntax in attributeListSyntax.Attributes)
            {
                ...
                if (fullName == "Shiny.Extensions.Reflector.ReflectorAttribute")
                {
    ...
    static void GenerateReflectorClass(SourceProductionContext context, ClassInfo classInfo, string accessorModifier)
    {
        ...
        sb.AppendLine($"{accessorModifier} class {classInfo.ClassName}Reflector : global::Shiny.Extensions.Reflector.IReflectorClass");
        sb.AppendLine("{");
        ...
        sb.AppendLine("    public global::Shiny.Extensions.Reflector.PropertyGeneratedInfo[] Properties => new global::Shiny.Extensions.Reflector.PropertyGeneratedInfo[]");
        sb.AppendLine("    {");
        for (int i = 0; i < classInfo.Properties.Count; i++)
        {
            var prop = classInfo.Properties[i];
            var comma = i < classInfo.Properties.Count - 1 ? "," : "";
            sb.AppendLine($"        new global::Shiny.Extensions.Reflector.PropertyGeneratedInfo(\"{prop.Name}\", typeof({prop.TypeForTypeOf}), {prop.HasSetter.ToString().ToLower()}){comma}");
        }
        sb.AppendLine("    };");
        sb.AppendLine();
    ...
    static void GeneratePartialTypeWithReflectorProperty(SourceProductionContext context, ClassInfo classInfo, string accessorModifier, bool isRecord)
    {
        ...
        sb.AppendLine($"{accessorModifier} partial {typeKeyword} {classInfo.ClassName} : global::Shiny.Extensions.Reflector.IHasReflectorClass");
        sb.AppendLine("{");
        sb.AppendLine("     private global::Shiny.Extensions.Reflector.IReflectorClass? _reflector;");
        sb.AppendLine($"    {accessorModifier} global::Shiny.Extensions.Reflector.IReflectorClass Reflector => _reflector ??= new {classInfo.ClassName}Reflector(this);");
        sb.AppendLine("}");
